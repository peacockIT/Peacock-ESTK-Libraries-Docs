	...
	Uint8: function(FileAsUint8Array){
		var file = {
			...
			readInt: function(_bytes){ 											// get integer from next _bytes group (big-endian)
				console.info("_bytes: "+_bytes+"; byteLength-pointer: "+(this.data.byteLength-this.pointer) );
				...
				if(_bytes > 1){
					for(var i=1; i<= (_bytes-1); i++){
						...
						console.info("value += ... "+value+" (pointer: "+this.pointer+") "+this.data.getUint8(this.pointer)+" "+Math.pow(256, (_bytes - i)));
						this.pointer++;
					}
				}
				value += this.data.getUint8(this.pointer);
				console.info("return value: "+value+" (pointer: "+this.pointer+")");
				this.pointer++;
				return value;
			},
		...
		MIDI.data 				= file.data;
		MIDI.headerSize 		= file.readInt(4);								// header size (unused var), getted just for read pointer movement
